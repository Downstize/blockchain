...
var handleBlockchainResponse = (message) => {
  ...
  if (latestBlockReceived.index > latestBlockHeld.index) {
    console.log(
      "blockhain possibly behind. We got: " +
        latestBlockHeld.index +
        "Peer got: " +
        latestBlockReceived.index
    );
    if (latestBlockHeld.hasg === latestBlockReceived.previousHash) {
      ...
      console.log("Recieved blockhain is longer than current blockchain");
      replaceChain(receivedBlocks);
    }
  } else {
    console.log(
      "Received blockchain is not longer than current blockhain. Do nothing"
    );
  }
};
...
///////////
var calculateHash = (previousHash, data, nonce) => {
  let hashValue = previousHash + data + nonce // Примерная реализация хэш-функции

  // Преобразование хэша
  let transformedHash = "";
  
  for (let i = 0; i < hashValue.length; i++) {
    const charCode = hashValue.charCodeAt(i);
    const transformedCharCode = charCode + 1; // Примерное преобразование символа

    transformedHash += (transformedCharCode % 16).toString(16);
  }
...
var isValidNewBlock = (newBlock, previousBlock) => {
  if (previousBlock.index + 1 !== newBlock.index) {
    ...
    console.log(
      typeof newBlock.hash + " " + typeofcalculateHashForBlock(newBlock)
    );
    console.log(
